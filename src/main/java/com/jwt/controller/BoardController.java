...
public class BoardController {
    ...
    public ResponseEntity<ApiResponse<Page<BoardDto.Response>>> getPosts(
        ...
        return ResponseEntity.ok(ApiResponse.ok(boardService.getPublicPosts(category, pageable)));
    }

    @GetMapping({"/api/posts/{postId}", "/api/board/{postId}"})
    public ResponseEntity<ApiResponse<BoardDto.Response>> getPost(@PathVariable Long postId) {
        try {
            return ResponseEntity.ok(ApiResponse.ok(boardService.getBoardById(postId)));
    ...
    public ResponseEntity<ApiResponse<BoardDto.Response>> createPost(
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.CREATED)
                    .body(ApiResponse.ok("글이 작성되었습니다.", responseDto));
        } catch (IllegalArgumentException e) {
